...
You will be given a list of rooms in a building (name, function, and the \
equipment inside each) and a search task. For EVERY room in the list, assign \
a likelihood score from 0.0 to 1.0 that the target of the task would be found \
in that room, based on the room's function and equipment — not just the room \
name. Include rooms with likelihood 0.0 if they are clearly irrelevant; do \
not omit any room.

Respond with ONLY a JSON object in this exact shape, no other text, no \
markdown fences:
{"rankings": [{"room": "<room name exactly as given>", "likelihood": <float 0.0-1.0>, "reasoning": "<one short sentence>"}]}
"""
...
def build_room_summaries(building):
    """Compact per-room text block for the prompt. Deliberately omits
    centroid_xy — proximity is a separate, later stage; giving raw
    coordinates to the LLM at the semantic-ranking step risks it treating
    spatial closeness as a relevance signal, which conflates two things
    that should stay independent (a room can be near and still wrong, or
    far and still correct)."""
    equip_by_room = {}
    ...
    for space in building["spaces"]:
        ...
        equip_str = ", ".join(equip) if equip else "(no equipment recorded)"
        lines.append(f"- {name} [{func}]: {equip_str}")
    return "\n".join(lines)
...
def validate_rankings(rankings, building):
    ...
    rule used throughout this pipeline."""
    known_rooms = {s["name"] for s in building["spaces"]}
    seen_rooms = {r.get("room") for r in rankings}

    missing = known_rooms - seen_rooms
    unknown = seen_rooms - known_rooms
    if missing:
        print(f"WARNING: model omitted {len(missing)} room(s) from its ranking: "
              ...
              file=sys.stderr)
        for room in missing:
            rankings.append({"room": room, "likelihood": 0.0, "reasoning": "(omitted by model)"})
    if unknown:
        print(f"WARNING: model returned {len(unknown)} room name(s) not in the building: "
              f"{sorted(unknown)}. Dropping these entries.", file=sys.stderr)
        rankings = [r for r in rankings if r.get("room") in known_rooms]
