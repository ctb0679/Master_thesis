...
def optimal_visit_order(candidates, centroid_lookup, start_room, max_exact=18):
    """Exact shortest Hamiltonian path from start_room through all candidates
    (no return to start required), via Held-Karp bitmask DP — O(n^2 * 2^n).
    Replaces the earlier greedy nearest-neighbor, which was empirically shown
    to produce paths ~25% longer than optimal on a real 5-candidate case
    (backtracks across the building because it commits to the locally-nearest
    room at each step with no lookahead).

    Falls back to greedy with a loud warning if candidate count exceeds
    max_exact — 2^n growth means this stops being cheap well before it
    becomes wrong; 18 candidates is ~4.7M states, already at the edge of
    "instant." Nothing in this building set (9-15 rooms total) should ever
    hit this fallback, but a distance-only silent regression here would be
    ...
    if n == 0:
        return []
    if n > max_exact:
        print(f"WARNING: {n} candidates exceeds max_exact={max_exact}, "
              f"falling back to greedy nearest-neighbor (not optimal). "
              f"Consider raising --threshold to reduce candidate count.",
              file=sys.stderr)
    ...
    FULL = (1 << n) - 1
    INF = float("inf")
    # dp[mask][i] = min cost to have visited exactly `mask`, ending at candidate i
    ...
    assert abs(raw_total - best_cost) < 1e-6, \
        "path reconstruction distance mismatch with DP cost — reconstruction bug"

    return ordered


def _greedy_nearest_neighbor(candidates, centroid_lookup, start_room):
    """Fallback only, used when candidate count exceeds optimal_visit_order's
    max_exact. Not used in normal operation."""
...
def main():
    ...
    plan = optimal_visit_order(candidates, centroid_lookup, args.start_room)
